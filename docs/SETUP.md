...
## 0. Посмотреть без базы

Если нужно просто увидеть продукт, база не нужна:
...
Пока не задан `NEXT_PUBLIC_SUPABASE_URL`, приложение работает в демо-режиме:
данные вымышленные и лежат в памяти процесса, вход — кнопкой без пароля,
правки живут до перезапуска. Всё остальное настоящее: те же экраны, та же
воронка, то же разделение доступа между агентством и клиентом.
...
3. Тем же способом `0002_rls.sql`, `0003_instagram_connect.sql`, `0004_creator_portal.sql`, `0005_offer_campaign_link.sql`. Порядок важен.
